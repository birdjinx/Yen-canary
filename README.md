...
## 일본 금리 수동 입력 (10년물·2년물)

일본 10년물은 FRED 월간(OECD) 근사치가 기본값입니다. 정확한 값을 쓰려면
대시보드에서 바로 확정할 수 있습니다. **일본 2년물**도 같은 방식으로 입력하면,
미일 금리차를 만기 일치한 **US2Y−JP2Y(정석)**로 계산합니다(미입력 시
US2Y−JP10Y 근사로 폴백).

**대시보드에서 확정 (원클릭)**
1. 일본 10년물 또는 2년물 카드의 입력칸에 실제 금리(%)를 넣습니다. 입력 후
   "미리보기"를 누르면 금리차·점수·등급이 화면에서 미리 계산됩니다.
2. **"수동값 확정 →"** 버튼을 누르면, 두 값이 이미 채워진 GitHub 편집
   화면이 새 탭에 열립니다(한쪽만 입력해도 다른 쪽 기존 값은 보존됩니다).
...
편집 화면이 바로 뜹니다. 근사치로 되돌리려면 편집 화면에서 해당 `value`를
`null`로 바꿔 커밋하면 됩니다.
...
**직접 편집도 가능**
`docs/manual.json`을 GitHub 웹에서 열어 고쳐 커밋해도 동일하게 동작합니다.

```json
{
  "jp10y": { "value": 1.62, "asof": "2026-08-14" },
  "jp2y":  { "value": 0.85, "asof": "2026-08-14" }
}
```

- `jp2y`가 비어 있으면(`null`) 금리차는 US2Y−JP10Y 근사로 폴백합니다.
- `jp10y`는 자국 회귀 유인 지표에도 별도로 쓰입니다.
